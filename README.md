...
Use the built-in logger anywhere in your app. Logs are automatically shipped to Loki and correlated with traces:
...
HTTP requests, database calls, and errors are instrumented automatically with no extra code. Health check endpoints (`/health`, `/healthz`, `/ping`) are ignored by default to keep traces clean.
...
## Using with a custom OTel backend

Observiz SDK uses standard OTLP gRPC — it works with any OTel-compatible backend:

```typescript
initObserviz({
  serviceName: "my-api",
  collectorUrl: "http://your-collector:4317",
})
```
